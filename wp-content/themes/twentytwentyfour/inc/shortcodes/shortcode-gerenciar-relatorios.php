...
function gerenciar_relatorios_shortcode()
{
	global $wpdb;
	...
	$protocolo_filter = isset($_GET['protocolo_filter']) ? sanitize_text_field($_GET['protocolo_filter']) : '';
	$itens_por_pagina = 10;
	...
	if (!empty($protocolo_filter)) {
		$query .= " AND numero_protocolo LIKE %s";
		$params[] = '%' . $protocolo_filter . '%';
	}
	...
	// ===== Total Pendente e Concluído =====
	$total_pendentes = (int) $wpdb->get_var("SELECT COUNT(*) FROM wp_relatorios WHERE status = 'pendente'");
	$abertas_hoje = (int) $wpdb->get_var("
    SELECT COUNT(*) 
    FROM wp_relatorios 
    WHERE DATE(hora_dia_ocorrencia) = CURDATE() AND status = 'pendente'
");

	$total_concluidos = (int) $wpdb->get_var("SELECT COUNT(*) FROM wp_relatorios WHERE status = 'concluido'");
				...
				<table class="table table-striped table-hover table-bordered">
					<thead class="thead-light">
						<tr>
							...
							<th style="width: 17%;">Implicado</th>
					...
					<tbody>
						...
						<?php foreach ($relatorios as $relatorio) :
							...
							if ($user_info && !empty($user_info->roles)) {
								$wp_roles = wp_roles(); // forma moderna e correta
								$nomes_cargos = array_map(function ($slug) use ($wp_roles) {
									return $wp_roles->roles[$slug]['name'] ?? ucfirst($slug);
								}, $user_info->roles);

								$cargo = implode(', ', $nomes_cargos);
							} else {
								$cargo = 'Não definido';
							}
						...
						?>
							...
							<tr>
								...
								<td><?php echo esc_html(ucfirst($relatorio->status)); ?></td>
								<td>
									<a href="<?php echo esc_url(add_query_arg([
													'relatorio_id' => $relatorio->id,
													'origem' => 'gerenciar-ocorrencia'
												], site_url('/detalhes-ocorrencia/'))); ?>" class="btn btn-primary">
										Detalhes
									</a>

									<?php if ($relatorio->status !== 'pendente') : ?>
										<a target="_blank" rel="noopener noreferrer" href="<?php echo $gerar_pdf_url; ?>" class="btn btn-dark btn-sm mt-1 w-100">RAAITO</a>
									<?php endif; ?>
								</td>
							</tr>
				...
				<ul class="pagination justify-content-center">
					<?php for ($i = 1; $i <= $total_paginas; $i++) : ?>
						<li class="page-item <?php echo ($i == $pagina_atual) ? 'active' : ''; ?>">
							<a class="page-link" href="?pagina=<?php echo $i; ?>&status_filter=<?php echo esc_attr($status_filter); ?>&user_id=<?php echo esc_attr($implicado_filter); ?>">
								<?php echo $i; ?>
							</a>
						</li>
					<?php endfor; ?>
...
add_shortcode('gerenciar_relatorios', 'gerenciar_relatorios_shortcode');
