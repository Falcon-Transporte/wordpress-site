...
const allowedTypes=["image/jpeg","image/png","image/gif"];for(let file of files){if(selectedFiles.length>=3)break;if(file.size>2*1024*1024){alert(`O arquivo "${file.name}" é muito grande! O tamanho máximo permitido é 2MB.`);continue}
if(!allowedTypes.includes(file.type)){alert(`Formato inválido para "${file.name}"! Apenas imagens JPEG, PNG ou GIF são permitidas.`);continue}
...
document.addEventListener("DOMContentLoaded",function(){const links=document.querySelectorAll(".link-indice");const chapters=document.querySelectorAll(".capitulo");const conteudoGrid=document.querySelector(".conteudo-grid");function hideAllChapters(){chapters.forEach(chapter=>chapter.classList.remove("ativo"))}
function showChapter(chapterId){const targetChapter=document.getElementById(chapterId);if(targetChapter){hideAllChapters();targetChapter.classList.add("ativo");conteudoGrid.scrollIntoView({behavior:'smooth',block:'start'})}}
links.forEach(link=>{link.addEventListener("click",function(e){e.preventDefault();showChapter(this.getAttribute("data-chapter"))});link.addEventListener("keyup",function(e){if(e.key==="Enter"){showChapter(this.getAttribute("data-chapter"))}})});if(chapters.length>0){chapters[0].classList.add("ativo")}});
