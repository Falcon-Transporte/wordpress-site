...
const form=document.getElementById("form-relatorio");form.addEventListener("submit",function(e){e.preventDefault();const formData=new FormData(form);selectedFiles.forEach((file)=>{formData.append("imagem_ocorrido[]",file)});fetch(form.action,{method:"POST",body:formData,}).then((res)=>{if(res.redirected){window.location.href=res.url}else{return res.text()}}).then((data)=>{if(typeof data==="string"){console.log(data)}}).catch((err)=>console.error("Erro ao enviar:",err))});
